import React from "react";
import Header from "@/components/Header";
import { Button } from "@/components/ui/button";
import { BsCheckCircleFill } from "react-icons/bs";

const Hero = () => {
  return (
    <div 
      className="relative min-h-screen w-full flex flex-col justify-between p-4 md:p-7 bg-cover bg-center"
      style={{ backgroundImage: "url('https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=1920')" }}
    >
      <div className="absolute inset-0 bg-black/80 backdrop-blur-sm"></div>
      <div className="absolute inset-0">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-yellow-400/10 via-transparent to-transparent"></div>
      </div>
      
      <div className="relative z-10">
        <Header />
      </div>

      <div className="relative z-10 flex flex-col lg:flex-row items-end justify-between gap-12 pb-16">
        <div className="w-full lg:w-[60%]">
          <div className="mb-6">
            <span className="text-yellow-400 text-sm uppercase tracking-wider">ACCOUNTING & BOOKKEEPING</span>
          </div>
          <h1 className="text-5xl md:text-6xl lg:text-7xl text-white font-normal tracking-tighter mb-6">
            Financial clarity that <span className="text-yellow-400">powers growth</span>
          </h1>
          <p className="text-white/80 text-lg md:text-xl leading-relaxed mb-8 max-w-3xl">
            Transform your financial operations with expert accounting professionals who bring 
            strategic insights, ensure compliance, and provide the real-time visibility you need 
            to make confident business decisions.
          </p>
          
          <div className="mb-12">
            <Button variant="yellow" size="lg" className="text-md">
              Get Started Today
            </Button>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div className="flex items-start gap-3">
              <BsCheckCircleFill className="text-yellow-400 text-xl mt-1" />
              <div>
                <p className="text-white font-medium">99.9% Accuracy</p>
                <p className="text-white/60 text-sm">Guaranteed precision</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <BsCheckCircleFill className="text-yellow-400 text-xl mt-1" />
              <div>
                <p className="text-white font-medium">50% Cost Savings</p>
                <p className="text-white/60 text-sm">vs. in-house teams</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <BsCheckCircleFill className="text-yellow-400 text-xl mt-1" />
              <div>
                <p className="text-white font-medium">24-Hour Support</p>
                <p className="text-white/60 text-sm">Always available</p>
              </div>
            </div>
          </div>
        </div>

        <div className="w-full lg:w-[35%]">
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-lg p-8">
            <h3 className="text-white text-2xl font-normal tracking-tight mb-6">
              Quick wins we deliver
            </h3>
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <span className="text-yellow-400 text-xl">→</span>
                <span className="text-white/90">Clean up years of messy books in weeks, not months</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-yellow-400 text-xl">→</span>
                <span className="text-white/90">Reduce monthly close time from weeks to 3 days</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-yellow-400 text-xl">→</span>
                <span className="text-white/90">Identify tax savings opportunities worth thousands</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="text-yellow-400 text-xl">→</span>
                <span className="text-white/90">Real-time financial dashboards for instant insights</span>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Hero;